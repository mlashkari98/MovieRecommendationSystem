...
import numpy as np
import copy
from GATAggregator import GATAggregator
...
class KGAT(torch.nn.Module):
    def __init__(self, num_user, num_ent, num_rel, kg, args, device):
        ...
        self.kg = kg
        self.device = device
        self.aggregator = GATAggregator(self.batch_size, self.dim, args.aggregator)
        self.linear = torch.nn.Linear(self.dim, 1)
        self.dropout = torch.nn.Dropout(0.2)
        self._gen_adj()
        self.user_linear = torch.nn.Linear(24, self.dim)
        self.ent_linear = torch.nn.Linear(20, self.dim)
    ...
    def forward(self, u, v):
        ...
        # [batch_size, dim]
        user_embeddings = self.user_linear(u).squeeze(dim = 1)
        ...
        return self.linear(self.dropout(scores))
